...
import {
  ...
  BarChart3, FileText, FileEdit, Settings, LogOut, Menu, X, BookOpenCheck,
} from "lucide-react";
...
export function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const currentKey = menuItems.find((m) => m.path === location.pathname)?.key || "dashboard";
          ...
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {sidebarOpen && <span className="whitespace-nowrap">{t(item.key)}</span>}
              </button>
            );
          })}
        </nav>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 bg-card border-b border-border flex items-center justify-between px-4 lg:px-6 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-foreground capitalize">{t(currentKey)}</h1>
          </div>
